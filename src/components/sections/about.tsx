...
import type { AboutSectionData, SiteLabels } from '@/types';

interface AboutProps {
  data: AboutSectionData;
  labels: Pick<SiteLabels, 'quickDetails' | 'locationText'>;
}

export function About({ data, labels }: AboutProps) {
  return (
    <div className="reveal grid grid-cols-1 gap-10 md:grid-cols-12">
      ...
      <aside className="md:col-span-5">
        <Card className="reveal">
          <h3 className="text-sm font-semibold">{labels.quickDetails}</h3>
          ...
          <div className="mt-6 border-t border-ink/10 pt-5">
            ...
            <p className="mt-1 text-sm font-medium">{labels.locationText}</p>
          </div>
        </Card>
      </aside>
    </div>
